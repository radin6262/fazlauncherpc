...
class _HomeScreenState extends State<HomeScreen> {
  final List<Game> games = [
    Game(
      ...
      windowsUrl: "https://github.com/radin6262/FazLauncherSite/raw/refs/heads/main/FiveNightsatFreddys.zip?download=true",
      image: "assets/images/fnaf1.png",
    ),
    Game(
        id: "fnaf2",
        name: "Five Nights at Freddy's 2",
        windowsUrl: "https://github.com/radin6262/FazLauncherSite/raw/refs/heads/main/FiveNightsatFreddys2.zip?download=true",
        image: "assets/images/fnaf2.png",
    ),
    Game(
        id: "fnaf3",
        name: "Five Nights at Freddy's 3",
        windowsUrl: "https://github.com/radin6262/FazLauncherSite/raw/refs/heads/main/FiveNightsatFreddys3.zip?download=true",
  ...
  Future<void> _loadConfig() async {
    ...
    setState(() {
      _bgImage = bg ?? 'assets/bg/background.gif';
    });
  }

  Future<void> _checkAllInstallations() async {
    for (var game in games) {
      final installed = await _gameService.isGameInstalled(game);
      setState(() {
        _installedMap[game.id] = installed;
      });
    }
  }

  Future<void> _handlePlay(Game game) async {
    final isInstalled = _installedMap[game.id] ?? false;
    ...
    if (isInstalled) {
      ...
      try {
        await _gameService.downloadAndInstall(game, (progress) {
          if (mounted) {
            setState(() {
  ...
  Future<void> _handleRemove(Game game) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${game.name} files removed.')),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error removing game: $e')),
        );
      }
    }
  }
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  image: DecorationImage(
                    image: AssetImage(_bgImage ?? 'assets/bg/background.gif'),
                    fit: BoxFit.cover,
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          IconButton(
                            icon: const Icon(Icons.collections, color: Colors.white70, size: 28),
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return GameCard(
                            ...
                            isInstalled: _installedMap[game.id] ?? false,
                            isDownloading: _isDownloading && _downloadingGameId == game.id,
                            ...
                            onPlay: () => _handlePlay(game),
                            onRemove: () => _handleRemove(game),
                          );
                        },
                      ),
                    ),
